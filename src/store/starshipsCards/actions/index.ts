...
import {
  ACTION_TYPE,
  StarshipsCardsAction,
  StarshipsCardsStoreCardsActionPayload,
} from './types';

export const storeStarshipsCardsAction = () => (dispatch: Dispatch) =>
  dispatch(getStarshipsAction())
    .then(({ payload }) => {
      ...
      const leftCard = (payload as GetStarshipsActionSuccessPayload).data.results[
        randomLeftNr
      ];
      const rightCard = (payload as GetStarshipsActionSuccessPayload).data.results[
        randomRightNr
      ];
