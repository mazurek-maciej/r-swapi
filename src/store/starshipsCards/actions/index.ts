...
export const storeStarshipsCards = () =>
  async (dispatch: Dispatch) => {
    ...
    const starships = await dispatch(getStarshipsAction())
    
    if (!starships.error) {
      const leftCard = (starships.payload as GetStarshipsActionSuccessPayload).data.results[randomLeftNr]
      const rightCard = (starships.payload as GetStarshipsActionSuccessPayload).data.results[randomRightNr]
